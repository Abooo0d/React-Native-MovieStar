...
export const getSearchResult = async (query: string):Promise<TrendingResult> =>{
  const response = await fetch(
    `https://api.themoviedb.org/3/search/multi?language=en-US&api_key=9dfd840e914a2c62795a7546d817bb3b&query=${encodeURIComponent(query)}`
  );
  const json  = await response.json();
  return json;
}
export const GetMovieDetails = async (id: number,type:MediaType):Promise<any> =>{
  const response = await fetch(
    `https://api.themoviedb.org/3/${type}/${id}?language=en-US&api_key=9dfd840e914a2c62795a7546d817bb3b`
  );
  const json  = await response.json();
  return json;
}